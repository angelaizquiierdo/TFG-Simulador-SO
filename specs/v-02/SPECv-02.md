...
- **Diagrama de Gantt — rama «¿y si?»:** se muestra **solo el diagrama de la rama**; el del escenario actual **no se repite** aquí porque ya aparece arriba en el simulador principal. Este diagrama tiene su **propio `PlaybackControls` independiente**, con su propia línea de tiempo (rango = longitud de la rama), separado del reproductor del simulador principal. Así la rama se reproduce por su cuenta y **siempre se ve completa**, dure más o menos ticks que el escenario actual. Las **métricas de comparación se muestran siempre** mientras la rama está activa, independientemente del tick.
...
> **Nota de implementación (v-02):** la rama *what-if* actual se obtiene **rederivando el escenario completo** con el algoritmo/parámetros alternativos (`run()` con *overrides*), no bifurcando desde el `SchedulerState` del tick `T` como describe el párrafo anterior. La bifurcación por estado (`runFrom`) queda como evolución futura; ver `DECISIONS.md`. La rama **comparte los procesos con el escenario actual**: al editar los procesos, la rama se rederiva con los nuevos procesos conservando su algoritmo/parámetros (si la edición la invalida, se descarta).
